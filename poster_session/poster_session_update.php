	...
	if($_FILES['file_pdf']['error']==4 and $_FILES['file_word']['error']==4){
		...
		if(!in_array($ext_word, $arr_ext_word)){
			...
			switch ( $_FILES['file_word']['error'] ){
				...
				break;
				default:
					if($row_data['ps_word']!=null){
						$delfile_name="../file/".$th."/". $row_data['ps_word'];
						unlink($delfile_name);
					}
					$sqlUpd = sprintf("UPDATE poster_session SET ps_word ='%s' WHERE ps_no=1", $file_name, 1);
		...
		if(!in_array($ext_pdf, $arr_ext_pdf)){
			...
			switch ( $_FILES['file_pdf']['error'] ){
				...
				break;
				default:
					if($row_data['ps_pdf']!=null){
						$delfile_name="../file/".$th."/". $row_data['ps_pdf'];
						unlink($delfile_name);
					}
					$sqlUpd = sprintf("UPDATE poster_session SET ps_pdf ='%s' WHERE ps_no=1", $th."poster_session.pdf", 1);
					$sqlU = mysqli_query($link, $sqlUpd) or die ("MYSQL Error");
				...
				default:
					switch ( $_FILES['file_pdf']['error'] ){
						...
						default:
							...
							$sqlUpd = sprintf("UPDATE poster_session SET ps_pdf='%s', ps_word='%s' WHERE ps_no=1", $th."poster_session.pdf", $file_name, 1);
							$sqlU = mysqli_query($link, $sqlUpd) or die ("MYSQL Error");
...
<!--class="form-control"-->
<form method="POST" action="poster_session_update.php?th=<?php echo $th; ?>" class="form-horizontal form-text-size" role="form" enctype="multipart/form-data">

<h1>修改海報議程</h1>
<hr>
<label class="control-label" >原始檔案</label><br>
<?php if($row_data['ps_pdf']!=null and $row_data['ps_word']!=null){ ?>
	<label class="control-label" >PDF</label><br>
	<button type="button" class="btn btn-link btn-sm" ><a Target="_blank" href="../file/<?php echo $th."/".$row_data['ps_pdf']; ?>">檔案預覽</a></button>　
	<button type="button" class="btn btn-link btn-sm" ><a href="../file/downloadfile.php?th=<?php echo $th; ?>&file=<?php echo $row_data['ps_pdf']; ?>">檔案下載</a></button>
	<input type="button" class="btn btn-primary" name="delete" id="delete" value="刪除" onclick="delect_Case(true, '<?php echo $row_data['ps_pdf']; ?>')">
	<br>
	<label class="control-label" >WORD</label><br>
	<button type="button" class="btn btn-link btn-sm" ><a href="../file/downloadfile.php?th=<?php echo $th; ?>&file=<?php echo $row_data['ps_word']; ?>">檔案下載</a></button>
	<input type="button" class="btn btn-primary" name="delete" id="delete" value="刪除" onclick="delect_Case(true, '<?php echo $row_data['ps_word']; ?>')"><br>
<?php }else{
	if($row_data['ps_pdf']!=null){ ?>
	<label class="control-label" >PDF</label><br>	
	<button type="button" class="btn btn-link btn-sm" ><a Target="_blank" href="../file/<?php echo $th."/".$row_data['ps_pdf']; ?>">檔案預覽</a></button>　
