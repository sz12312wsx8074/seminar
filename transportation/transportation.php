...
<h1>交通指南</h1>
<hr>
<form method="POST" class="form-horizontal form-text-size" action="transportation.php?th=<?php echo $th; ?>" enctype="multipart/form-data">
<?php if($totalRows_data==0){ 
	echo "目前還沒有資料"; ?>
	<br><br>
	<?php if($button_on){ ?>
	<input type="button" class="btn btn-primary" name="insert" value="新增交通指南" onClick="self.location='transportation_insert.php?th=<?php echo $th; ?>'">
	<?php } ?>
	<?php  }else{ ?>
	路線說明：<?php echo $row_data['tr_content']."　";?>
		<br>
	地址：<?php echo $row_data['tr_address']; ?>
	<br>
		...
		<br>
	<?php if($button_on){ ?>
		<input type="button" class="btn btn-primary" name="update" value="修改" onClick="self.location='transportation_update.php?th=<?php echo $th; ?>'">
	<?php } ?>
    <?php
        //-----Google map value Start-----
        $set_address=$row_data['tr_address']; //填寫所要的地址，Example地址為勤美綠園道
