...
$query_where = "SELECT * FROM important_date Where where_from=1";//時間設定來的
...
<form class="navbar-form nav-text" role="search" method="POST" action="important_date.php?th=<?php echo $th ?>">
<body>
...
<?php
if($button_on){ ?>
	<input type="button" class="btn btn-primary" id="contant" name="contant" value="修改敘述"  ONclick="self.location='contant.php?date_no=<?php echo $row_data["date_no"];?> & th=<?php echo $th ?>'"><br><br>
<?php } ?>
<?php echo $row_data1['contant']; ?><br>
<hr>
<?php 
if($button_on){ ?>
	<input type="button" class="btn btn-primary" name="insert" id="insert" value="新增" onClick="self.location='insert.php?th=<?php echo $th ?>'">
	<input type="button" class="btn btn-primary" name="dele" id="dele" value="刪除選取" onClick="delete_Case(false, '')" /><br><br>
<?php } ?>
<div class="form-group">
	...
	<input type="button" class="btn btn-default" id="view" name="view" value="總覽" onClick="self.location='important_date.php?th=<?php echo $th ?>'">
...
<?php 
$sum = 0 ;
if($row_meeting['date_date']==null){ ?>
<tr>
	<td></td>
	<td><?php echo $row_meeting['date_content'];?></td>
	<td>尚未設定日期</td>
	<?php 
	if($button_on){ ?>
		<td  colspan="2"><input type="button" class="btn btn-info" name="update" id="update" value="設定" onClick="self.location='update.php?date_no=<?php echo $row_meeting["date_no"]; ?> &th=<?php echo $th ?>'"></td><?php
	}else{ ?>
		<td colspan="2"></td><?php 
	} ?>
</tr><?php
...
do {
	if($row_data['where_from']==3 and $row_data['date_date']==null){
		$have = false;
	}
	
	if($row_data['date_no'] != $no and $row_sum !=0 and $have){
	?>
	<tr>
		<?php
		if($button_on){ 
			if($row_data['where_from']!=1 and$row_data['where_from']!=3){ ?>
				<td><input class="choose" type="checkbox" name="<?php echo $row_data['date_no']; ?>" /></td><?php
			}else{ ?><td></td>
		<?php } 
		}else{ ?>
			<td></td>
		...
		<?php 
		if($button_on){ 
			if($row_data['where_from']!=1){ ?>
				<td><input type="button" class="btn btn-info" name="update" id="update" value="修改" onClick="self.location='update.php?date_no=<?php echo $row_data["date_no"]; ?> &th=<?php echo $th ?>'"></td><?php
				if($row_data['where_from']!=3){ ?>
					<td><input type="button" class="btn btn-info" id="delete" name="delete" value="刪除" onclick="delete_Case(true, '<?php echo $row_data['date_no']; ?>','<?php echo $th ?>')"></td><?php
				}else{ ?>
					<td></td><?php
				} 
			}else{ ?>
				<td colspan="2"><font color="red">請至時間設定修改</font></td><?php
			}
		}else{ ?>
			<td colspan="2"></td><?php 
		} ?>
...
<script>
function delete_Case(single, no, th) {
	var dele = confirm("確定要刪除選取項目？");
	if (dele == true){
		if (single){
			location.href='delete.php?date_no='+no+'&th='+th;	
		}else{
