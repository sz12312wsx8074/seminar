...
$using = "SELECT * FROM current_committee";

$sql_new = "SELECT cc_email FROM current_committee where current_committee.cc_email = '$using' ORDER BY cc_email DESC";
...
$email = $row_data_new['cc_email'];
$committee_content = $row_data['committee_content'];
...
if(!$mail->Send()){
	...
	exit ('<meta http-equiv=REFRESH CONTENT=0.1;url=list.php?up_no='.$row_data_new['up_no'].'>');
} ?>