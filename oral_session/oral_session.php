...
<?php if($totalRows_data==0){ ?>
	...
	<br><br>
	<?php if($button_on){ ?>
		<input type="button" class="btn btn-primary" name="insert" value="新增發表議程" onClick="self.location='oral_session_insert.php?th=<?php echo $th; ?>'">
	<?php } ?>
<?php }else{ 
	if($row_data['os_pdf']!=null){ ?>
		<label class="control-label" >PDF</label><br>
		<a Target="_blank" href="../file/<?php echo $th."/".$row_data['os_pdf']; ?>">檔案預覽</a>　
		<a href="../file/downloadfile.php?th=<?php echo $th; ?>&file=<?php echo $row_data['os_pdf']; ?>">檔案下載</a>
		<br>
	<?php }
	if($row_data['os_word']!=null){ ?>
		<label class="control-label" >Word</label><br>
		<a href="../file/downloadfile.php?th=<?php echo $th; ?>&file=<?php echo $row_data['os_word']; ?>">檔案下載</a>
	<?php } ?>
	<br><br>
	<?php if($button_on){ ?>
		<input type="button" class="btn btn-primary" name="update" value="修改" onClick="self.location='oral_session_update.php?th=<?php echo $th; ?>'"><br><br>
	<?php } ?>
<?php } ?>
