...
if(isset($_POST['update'])){
	if($_FILES['file_pdf']['error']==4 and $_FILES['file_word']['error']==4){
		...
		exit ("<meta http-equiv=REFRESH CONTENT=0.1;url=oral_session.php?th=$th>");
	}elseif($_FILES['file_pdf']['error']==4 and $_FILES['file_word']['error']!=4){
		$ext_word = strrchr($_FILES['file_word']['name'],'.');
		$file_name= $th."oral_session".$ext_word;
			...
			switch ( $_FILES['file_word']['error'] ){
				...
				default:
					if($row_data['os_word']!=null){
						$delfile_name="../file/".$th."/". $row_data['os_word'];
						unlink($delfile_name);
					}
					$sqlUpd = sprintf("UPDATE oral_session SET os_word ='%s' WHERE os_no=1", $file_name, 1);
		...
		if(!in_array($ext_pdf, $arr_ext_pdf)){
			...
			switch ( $_FILES['file_pdf']['error'] ){
				...
				break;
				default:
					if($row_data['os_pdf']!=null){
						$delfile_name="../file/".$th."/". $row_data['os_pdf'];
						unlink($delfile_name);
					}
					$sqlUpd = sprintf("UPDATE oral_session SET os_pdf ='%s' WHERE os_no=1", $th."oral_session.pdf", 1);
					$sqlU = mysqli_query($link, $sqlUpd) or die ("MYSQL Error");
					
					if($sqlU){
						move_uploaded_file($_FILES['file_pdf']['tmp_name'], "../file/".$th."/". iconv("utf-8", "big5", $th."oral_session.pdf"));
						echo "<script>alert('已修改!!');</script>";
						exit ("<meta http-equiv=REFRESH CONTENT=0.1;url=oral_session.php?th=$th>");
					}
			}
				...
				default:
					switch ( $_FILES['file_pdf']['error'] ){
						...
						default:
							...
							$sqlUpd = sprintf("UPDATE oral_session SET os_pdf='%s', os_word='%s' WHERE os_no=1", $th."oral_session.pdf", $file_name, 1);
							$sqlU = mysqli_query($link, $sqlUpd) or die ("MYSQL Error");
...
<!--class="form-control"-->
<form method="POST" action="oral_session_update.php?th=<?php echo $th; ?>" class="form-horizontal form-text-size" role="form" enctype="multipart/form-data">

<h1>修改發表議程</h1>
<hr>
<label class="control-label" >原始檔案</label><br>
<?php if($row_data['os_pdf']!=null and $row_data['os_word']!=null){ ?>
	...
	<input type="button" class="btn btn-primary" name="delete" id="delete" value="刪除" onclick="delect_Case(true, '<?php echo $row_data['os_pdf']; ?>')">
	<br>
	...
	<input type="button" class="btn btn-primary" name="delete" id="delete" value="刪除" onclick="delect_Case(true, '<?php echo $row_data['os_word']; ?>')"><br>
<?php }else{
	if($row_data['os_pdf']!=null){ ?>
	<label class="control-label" >PDF</label><br>
	<button type="button" class="btn btn-link btn-sm" ><a Target="_blank" href="../file/<?php echo $th."/".$row_data['os_pdf']; ?>">檔案預覽</a></button>　
