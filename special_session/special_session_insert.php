	...
	if($_FILES['file_pdf']['error']==4 and $_FILES['file_word']['error']==4){
		...
		if(!in_array($ext_word, $arr_ext_word)){
			...
			switch ( $_FILES['file_word']['error'] ){
				...
				break;
				default:
					$sqlIns = sprintf("INSERT INTO special_session (ss_word) values (%s)", '"'.$file_name.'"');
		...
		if(!in_array($ext_pdf, $arr_ext_pdf)){
			...
			switch ( $_FILES['file_pdf']['error'] ){
				...
				break;
				default:
					$sqlIns = sprintf("INSERT INTO special_session (ss_pdf) values (%s)", '"'.$th."special_session.pdf".'"');
					$sqlI = mysqli_query($link, $sqlIns) or die ("MYSQL Error");
			...
			switch ( $_FILES['file_word']['error'] ){
				...
				default:
					switch ( $_FILES['file_pdf']['error'] ){
						...
						break;
						default:
							$sqlIns = sprintf("INSERT INTO special_session (ss_pdf, ss_word) values (%s, %s)", '"'.$th."special_session.pdf".'"', '"'.$file_name.'"');
							$sqlI = mysqli_query($link, $sqlIns) or die ("MYSQL Error");
...
<!--class="form-control"-->
<form method="POST" action="special_session_insert.php?th=<?php echo $th; ?>" class="form-horizontal form-text-size" role="form" enctype="multipart/form-data">
